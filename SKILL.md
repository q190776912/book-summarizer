...
- **🔴 不允许无中生有的「结构」（2026-08-02 立）**：除了不能编造内容，同样**不能编造版面结构**——不得新建原书没有的标题层级、不得把原书穿插排布的内容抽出来归拢成块、不得重排条目顺序。总结文件的骨架（节的个数与顺序、条目与练习的先后位置）必须与原书页码顺序**逐条同构**。（真实事故：Vakil《The Rising Sea》全 24 个文件因自造节末 `### 练习` 归拢块而整批报废，见「练习/习题收录规则」。）
  ...
  - **🔴 非核心内容必须「摘要」而非「整段照抄」（2026-08-02 立，2026-08-02 细化）**：总结是**摘要**，不是书的副本。按内容类型分三档处理（详规见「保真分级与摘要策略」）：① **Tier 1 高保真**——定义/定理/引理/推论/命题/公理的**陈述**、**例子**的**陈述（题面）**、**练习**的**题目**、**注记（Remark/Aside）**，须原汁原味（注记**保留完整、少修改**，只修 OCR 噪声）、保留关键公式与精确术语；② **Tier 2 摘要**——动机/直观/背景/过渡/导语等描述性内容须概括为核心要点（2–4 句）或干脆省略，不得把原书段落整段搬来；③ **Tier 3 证明/解答只列核心步骤**——证明与例子的**解答/证明**不逐字重写，只列**核心步骤**：用 `1. 2. 3. …` 按序标号（**步数按实际需数，不限定 3 条**；「1,2,3」只是示意写法），每步一句话点出关键动作（关键引理 / 恒等式 / 归纳 / 构造 / 交换图），结尾给一句结论。**判据：某段散文超过 ~4 句且只是在复述原书文字 = 你在抄而非在总结，必须压缩。**（真实事故：Vakil《The Rising Sea》前几章被写成书的逐段英文副本，丧失摘要价值；verify **P 层 `p_verbose` / `p_proof_verbose`** 会据此判 FAIL。）
...
- **练习题（Exercise）**：练习的**题目陈述**——原样保留（OCR 噪声按语义重写为正确 LaTeX，但题意、条件、设问不得概括或删减）。
...
### Tier 2 — 摘要（概括核心要点，禁止整段照抄）
以下内容的**叙述性散文**必须**概括为核心要点（2–4 句）**，不得把原书段落整段搬到总结里（**注记不在此列**，见 Tier 1）：
- 动机（Motivation）、直观（Intuition）、背景 / 历史讨论、一般性铺陈、连接性过渡段：概括为核心要点，**可省略**。
- 章节导语、节首引言：概括为核心要点，**可省略**。
- ⚠️ 若某段散文超过 ~4 句且只是在复述原书文字，说明你在抄而非在总结，必须压缩。
...
**规则 E — 禁止阻塞式逐章确认，必须一口气跑完全书（最高优先级，凌驾于任何「逐章汇报」冲动之上）**：任务一旦启动，Agent 必须**连续**把 `chapter_map` 中的**全部剩余章节**（对双语书即每章中文+英文两版）全部写完并验证通过，**中间不得停下询问用户「是否继续 / 推进下一章吗」、不得逐章等待人工确认、不得将「单个章节完成」当成一次回合的结束点**。唯一的任务结束信号是 `已写章数 == 总章数` 且每一章都 `verify PASS + KaTeX OK + audit_counts PASS`（条数核对一致）。章与章、文件与文件之间**无缝衔接**：上一章校验一通过，立刻动手写下一章，全程不向用户发「继续？」之类的追问。若某章一次 verify 未过，用 `--fix` 或 `_fix_*.py` 等脚本自动修复后重验；仍不过则继续修（最多重验 2 次），**严禁停下来问用户**。全书完成后的最终汇报一次性给出。

**规则 F — 源语言优先（双语书：源语言版先写、先校验、先修复；翻译版后派生）**：
- **英文原著书**：先写**英文版** `ChapterN_*.md` 作为「源文件」→ 按 Step 3 / Step 3.5 / Step 4 把它**完全校验、完全修复至 `verify PASS + KaTeX OK`** → **之后**才据这份已校验的英文版**翻译**出中文版 `第N章_*.md`。中文版是英文版的派生，不是与英文平行、各写各的独立文件。
- **中文原著书**：仅出中文版，无此顺序问题。
...
- 定义/定理/命题/推论/引理/断言/公理 → `**粗体标签**：`（禁止用 `###`）—— **gm 体例书（条目按小节标题排印的书，如 Gelfand-Manin）例外，见下方「gm 条目标题规则」**
- **⚠️ 条目编号必须 1:1 照搬原书印刷号，严禁自创「节.条」编号（最高优先级编号规则）**：
  - 标签里的数字必须**等于原书该条目印刷时印出的编号**，不得拼接「节号.条号」。原书怎么印就怎么写：原书印 `DEFINITION 1` / `定义 1` → 写 `**定义 1**`；原书印 `THEOREM 2` → 写 `**定理 2**`。**绝不允许**写成 `**定义 1.1**`、`**定理 6.1**` 这种「节号.条号」合成编号——原书根本没有这种编号。
  - Rosen《Discrete Mathematics》等书的编号规则是：**每个 `§N.M` 节内，定义/定理/引理/推论/命题/公理各自从 `1` 开始独立计数、且节与节之间重新归 `1`**（即 §1.1 有 Definition 1,2,3…，换到 §1.3 又从 Definition 1 重新开始；例/Example 同理每节归 1）。写 `**定义 1**` 而非 `**定义 1.1**`。
  - `verify` 工具内部的条目键（如 `6.1-1`、`7.4-8`）只是**校验用的机器键，绝不能泄漏成展示标签**。展示标签只写原书印刷号。
  - 若原书某节确实用「章.条」式编号（如 do Carmo《Riemannian Geometry》的 `Lemma 3.2`、Atiyah-Macdonald 的 `Proposition 1.4`），则照原书印法写，不要改成每节归 1。规则核心永远是「**照搬原书印刷号**」，不是「一律每节归 1」。
  - **🔴 number-first 体例（编号在前、标题在后、多数条目无类型词的书，如 Vakil《The Rising Sea》）——印刷标题必须保留进标签（2026-08-02 立）**：这类书把条目印成 `1.2.1. Categories.`、`1.2.10. Definition.`、`1.2.4. Example: abelian groups.`，即**编号后面紧跟一个句点标题**，而不是 `Definition 1.2.1` 这种类型词在前的写法。规则：
    - 该句点标题是**条目名**，必须写进粗体标签的括号里，**绝不允许丢弃后扔进正文**。
    - 无类型词的条目 → 中文版 `**1.2.1（范畴 (Categories)）：**`，英文版 `**1.2.1 (Categories)**:`。
    - 印刷标题本身就是类型词（`1.2.10. Definition.`）→ 按常规译成类型标签：`**定义 1.2.10**：`；若类型词后还带主题（`1.2.10. Definition (subcategory).`）→ `**定义 1.2.10（子范畴 (subcategory)）：**`。
    - 印刷标题是 `Example` / `Important Example` / `Topological example` → 按「例」处理（进 `>` 块引用）；是 `Remark` / `Aside` → 按「注」处理（进 `>` 块引用）。
    - ❌ 严禁产出裸标签 `**1.2.1**` 后面另起一行写 `Categories. A category consists of…`——这会让读者完全看不出这条讲的是什么。
    - 若某条目原书确实没有印标题（纯 `1.2.1.` 后直接接正文），才允许写裸编号标签，并在括号里补一个**你自拟的、能概括该条内容的**短标题：`**1.2.1（态射的合成律）：**`。
  - 编号必须与原书条目数一致：原书某节有 N 条定义就写 N 条，多写或少写都违反「不允许无中生有 / 不得遗漏」。
  - **⚠️ 条数核对是强制收尾环节（兜底「不允许遗漏」）**：每章写完 `verify PASS + KaTeX OK` **之后**，还必须跑 `python verify/audit_counts.py <ch> <start> <end> <md_file> <extract_dir>` 逐节核对「各节定义/定理/引理/推论/命题/公理的条数 == 原书该节该类型条数」。脚本报 `FAIL`（缺漏/多余/编号不连续）则**该章不算完成**，须补全条目后重跑直到 `PASS`。注意：`verify` 工具只校验章节标题键、**不查条数**，所以 `audit_counts.py` 是「不允许遗漏」这条规则的唯一自动化兜底，绝不可跳过。
- **⚠️ 章节标题 `§N.M` 是合法的**（节/子节标题本来就该带 §），上述规则只约束「定义/定理等条目标签里的数字」，不约束节标题。
- **gm 条目标题规则（2026-08-01 立，罗马数字章 + 每节内条目从 1 起号、条目按小节标题排印的书，如 Gelfand-Manin《Methods of Homological Algebra》）——条目小节标题写 `###`，照原书排版**：原书把条目印成小节标题的（`1. Main Definitions`、`3. Proposition.`、`6. Definition (- Lemma).`），总结文件写成 ATX 标题 `### ` + 原书标题文字原样（`### 1. Main Definitions`）；不加粗、不加冒号、不增删标点（原书标题带句号就带句号）。节标题仍是 `## §N. ...`。**禁止**把这类条目标题写成 `**N. Title**` 内联加粗，也禁止拼造「节.条」编号（如 `### 1.1 Main Definitions`——原书每节内从 1 起号，照原书印法写）。**子节标题（`### N. Title`）下方不得紧接 `---` 分隔线**（标题本身就是条目分隔，其下直接接内容/子点；`---` 只允许在条目与条目之间——即上一个条目的内容与下一个 `###` 标题之间）。机器键仍为 `标签I.S-N`（verify 的 gm scheme 自动生成，`### N. Title` 与旧 `**N. Title**` 两种形式均识别；**中文版沿用同一 scheme**：条目标题写成 `### N. 中文标题`，标题内标签词用中文（定义/定理/引理/推论/命题/例/注记…），`GM_HEAD_LABEL_RE` / `GM_LABELED_RE` 同时接受中英标签词并统一规范到同一机器键，正文引用照写如「引理 I.2.11」）。结构性标签（Definition/Proposition/…）出现在 `###` 标题内时，陈述仍须顶层独立成行、证明/例仍进块引用，其余全部格式规则不变。
...
- **证明思路/梗概/概要 / 例子的解答** → `> **证明思路**：` / `> **解答**：` 块引用内，多步骤用 `1. 2. … N.` 按序标号（**Tier 3 政策**：证明与例子解答只列核心步骤，步数按实际需数、不限定 3 条；每步一句话点出关键动作，不得逐字重写/逐段翻译原书 proof；详见「保真分级与摘要策略」）
...
- 首次出现的关键术语标注 `(English)`
- **练习/习题收录规则（2026-07-31 立，2026-08-02 加硬约束）**：判定标准是有无**专门的习题小标题**（如「练习」「习题」「Exercises」「Problems」等）把习题归拢成块。(a) **带有专门习题小标题的集中习题块**（如 `### 练习`、`## §1.11 习题`、`**习题**` 等标题下罗列的编号习题）→ **一律省略，不写入总结**，也不写「依规则略去」之类注记，直接跳过。整节都是习题的节同理。(b) **没有专门习题标题、以穿插形式出现的习题**（习题散落在定义/定理/例等正文条目之间，或虽在节末但没有用「练习/习题」标题归拢）→ **保留**（可以是一道或多道），在原位写出题目（OCR 噪声必须按语义重写正确 LaTeX），可附一句提示。简言之：**有标题归拢即省，无标题穿插即留**。
  - **🔴「原位」= 原书页码顺序上的那个位置，不是节末（硬规则，违者该章判废）**：穿插习题必须**夹在它前后的正文条目之间**，与原书排印顺序逐条对齐。**严禁**把散落的穿插习题抽出来、在节末或章末新建 `### 练习` / `### 习题` / `**练习**` 之类的**归拢块**集中罗列——那等于把原书结构重排了一遍，属于「无中生有的结构」，与规则 (a) 要省略的形态恰好是同一种东西（区别只在于一个是原书自带、一个是你造的，而你造的更糟）。
  - ✅ 正确（原书 §1.2 排印顺序：`1.2.3` → 练习 1.2.A → 练习 1.2.B → `1.2.4` → …）：
    ```
    **1.2.3（态射的合成）：** …
    **练习 1.2.A：** …
    **练习 1.2.B：** …
  ...
  - ❌ 错误（把练习拔出来堆到节末）：
    ...
    **练习 1.2.B：** …
    ```
  - **练习标签格式**：与正文条目同级顶层写 `**练习 N.M.X（Exercise N.M.X）：**`（中文版）/ `**Exercise N.M.X**:`（英文版），紧接题目正文。练习**不进块引用**（除非原书把它印在例/证明内部），不计入 verifier 必备条目（`ch*_items.txt` 不含练习）。
  - **禁止吞并**：一道练习的正文到下一个正文条目编号出现即结束。绝不允许把后续的 `1.2.4`、`1.2.5`… 一路粘进同一段练习文字里（真实事故：Vakil《The Rising Sea》Ch1 曾把 `1.2.4`–`1.2.22` 共 19 个条目全部吞进「练习 1.2.B」的段落中）。
...
1. **🔴 结构骨架（写作契约，2026-08-02 立为强制）**：
   ```
   python extract/scan_skeleton.py <extract_dir> <ch>        # 三级编号书，默认
   python extract/scan_skeleton.py <extract_dir> <ch> --scheme two-level
   ```
   ...
   - 每个 `ITEM` / `EXER` 都必须在总结里落地，且**相对先后顺序与骨架一致**（骨架顺序 = 原书页码顺序 = 练习该待的「原位」）；
...
2. **编号项清单**：按 [`references/formatting.md#提取流程`](references/formatting.md#提取流程) 的 9 步流程从 JSON 提取，与骨架交叉核对后写作。
...
| 15 | **非核心内容已摘要（P 层 verbose 闸门）** | `verify/verify_chapter.py` 的 **P 层** 统计「顶层长散文段」（`p_verbose`，不含 `**` 标签条目/例/练习/注记的忠实内容）与「过长且未分条的证明/解答块」（`p_proof_verbose`）；超标（`p_verbose` ≥ 6 段 或 `p_proof_verbose` ≥ 2 块）即判 FAIL。⚠️ 已用 `1. 2. 3. …` 分条枚举的证明【步数不限】一律豁免；**注记（Remark/Aside）与例题面一样按 Tier 1 完整保留（少修改），不参与 verbose 判定**——只有「整段散文墙」式的证明/解答才会被拦。证明/例子解答须压成核心步骤、动机/导语须概括为核心要点或省略（详见「保真分级与摘要策略」） |
