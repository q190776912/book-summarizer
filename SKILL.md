...
<corpus_root>\<书名>\       ← 每个书一个文件夹（corpus_root 见 user_config.json / README 配置章节）
  ...
  │   └─ book_structure\    ← 内容化分章契约 ch{N}.json（附录 appendix{X}.json）+ units\ch{N}\（附录 units\appendix{X}\）每 item 一单元的拆分目录（write-source 步骤 4）
  ├─ 第1章_章名.md           ← 中文版
  ├─ Chapter1_Name.md       ← 英文版（仅英文/他语种书时有）
  └─ ...
```

> **附录命名总则（全局）**：附录的一切随章命名一律用 `appendix{X}`（X=A..Z，取自 chapter_map 字母章键），**"ch" 只属于数字章**——分章契约 `appendix{X}.json`、单元目录 `units\appendix{X}\` / `units-translate\appendix{X}\`、完整性报告 `appendix{X}_completeness_report.json`、随章侧车文件 `ignore_appendix{X}.json` / `manual_overrides_appendix{X}.json`、图像基名 `appendix{X}_fig*.png`、日志/报告显示标签亦为 `appendix{X}`（`lib` 级判据 = `data/book_structure/book_structure.py` 的 `chapter_label`，与 `chapter_json_name` / `unit_dir_name` 同源）。原书后部附录（结构契约章节点 `name` 含 `Appendix X`，或章键为字母）最终 md 按附录命名——英文 `AppendixX_Name.md`、中文 `附录X_中文名.md`（Name 取自契约章名，不重排号）；`chapter_md_groups` / flow 物理证据均已识别该形态。**无编号附录（有序标缺失，禁止伪造）**：原书附录若**没有印刷序标**（既非 `Appendix A` 也非数字号），**严禁**按「第 N 节」补一个伪造的数字序标（如把它登记成 `ch7` / `appendix7` / `附录7`）。正确做法：chapter_map 里该章**键直接写 `appendix`**（`kind=2`，`num`/键 = `"appendix"`，`name` 只放裸标题如 `附录`）——命名 SSOT `chapter_ordinal` 见「键 == 前缀词」即把序标归**空**，于是契约 `appendix.json`、单元目录 `units\appendix\`、标签 `appendix`、H1 `# 附录`、最终 md `附录.md`（英文书 `Appendix.md`）一律退回**裸名**，不带任何序标；判据与字母附录/数字章同源（`data/book_structure/book_structure.py`）。补篇无号同理写键 `supplement`。内部标题体系照旧用原书序标（如 `## §A.1`、`**Proposition A.1**`；无号附录正文直接用 `**定理 N**` 等原书条目号，不冠章号）。
...
| 2 | [`write-source`](flows/write-source/write-source.md) | 步骤 1 **config 子流程**（chapter_map + `verify_config.json`）→ 步骤 2 **figure_detection 子流程**（图检测+分配）→ 步骤 3 **structure 子流程**（`build_structure` 按章**一步产出含内容的完整契约** `ch{N}.json`（层级嵌套小节 + description/proof/text/formula含tag/image 内容块）+ 🔴 章节/条目完整性查漏回填闸门，`gate.passed` 才放行；回填后自动重建该章内容）→ 步骤 4 **拆分单元**（纯脚本：内容完整性闸门 `check_content_completeness` + `split_draft_units` 拆出 `units/ch{N}/`（附录 `units/appendix{X}/`）每 item 一单元目录）→ 步骤 5 **逐个按写作要求改好（agent 核心步）+ 强制门控**（agent **逐个改好每个单元**：写作要求全部在此落实（公式重写 / Tier 压缩 / 格式），并把首行 DRAFT→DONE → 🔴 `gate_units` 强制门控：每单元存在+DONE+**单元级质量校验通过**（公式闭合 / 围栏形态（单行 `$$` 块 / 缺空行·空 `>` 行 / `\tag` 出块外）/ 无裸命令·裸 Unicode 字符·裸箭头（数学必须 KaTeX）/ 结构标签 / 块引用结构（嵌套 `> >`、例/证明断裂、`>` 块无标签或标签未包 `>`、列表后无空行、连续空 `>` 行，复用 verify F/H 层九项）/ 无 OCR 残留 + 内容审阅残留（QED 框「口」/ 乱码重复片段 / 编码损坏 / 单元内私造标题）/ **契约 tag 对账**（缺编号公式 = 漏写、多出 = 编造，均不通过）/ **契约 ↔ manifest 双向对账 + 标签须契约在账**（正向 = tag/图片对账；反向章级闸 ⑩ = `unit_node_entries` 真值，契约里每个应成单元的节点都须有单元记录，按「习题 / 结果项」分桶 + `sec_ordinals` 序标归一比较，漏记录 = 该条内容 merge 后整条消失；质量校验第 23 项 = 行首粗体头 `**2.4.3.**` / `**Exercise 13.3.3\***` / `**习题 13.3.3**` 里的三级序标必须是契约键（`chapter_ordinals` 真值），拦「OCR 把跨节习题整段挂成 description → 单元有题面而契约无条目」）/ **契约图片对账 + 空正文闸 + 章级图片覆盖**（manifest 按契约节点写入 `images`/`content`：缺图 = 内容丢失、多图 = 编造/错位；契约节点有内容块而正文清空 = 不通过，幻影节点 content=0 允许空；契约任一图全无单元嵌入或单元嵌契约外图 = 章级 FAIL，`unnamed` 占位图与同章标号图别名配对）+ **真实 KaTeX 渲染**（按章批量，错误映射回单元），判断标准是「写对」而非「重写」，**校验执行失败 = 不通过（fail-closed）**）才 exit 0（每个 item 都不漏））→ 步骤 6 **逐单元翻译**（仅英文书；`init_translate_units` 初始化清单（元数据+src_hash，不复制正文）→ agent 逐个看一个源单元生成一个翻译单元 `units-translate/ch{N}/`（附录 `units-translate/appendix{X}/`）（DONE）→ 双重门控 `gate_units --units-dir units-translate`（🔴 翻译前置硬闸：先重跑源章门控，源单元未全部修正完成即拒绝——源先于译，防译文基于旧源作废）+ `check_translate_parity` 1:1 同构闸（🔴 src_hash 漂移/快照缺失均 FAIL））→ 步骤 7 **拼接**（`merge_units --all` 纯脚本机械执行，一次拼源+译两组 `ChapterN_*.md` / `第N章_*.md` 并按 V-F 重建 `---` 分隔线，**无 agent 参与**；🔴 拼接后合并形态章 md >60000 字符必须随即跑 `tools/split_chapters.py` 按节拆分，否则 `merge_all` 证据复核硬拒 mark（规则3 机械闸）；🔴 落账证据强制「逐单元改好+门控通过」+ 契约骨架节/编号项在位，脱离单元 = mark 硬拒）→ 步骤 8 批量校验至 PASS（一次覆盖源+译两版；🔴 含 B/O 层序标校验，缺号缺口经 `backfill_ordinals.py` 回填归属单元） | config 在 MM 修复后统一生成只一次（config_setting 步骤 1）；🔴 **图检测前必须 config 先行**；🔴 **structure 完整性闸门是拆分的硬闸（规则7）**；内容完整性闸门（描述/证明/图片/文字块齐备）在步骤 4；源语言优先（规则1）；拆章（规则3）；写源期间**禁逐章 verify（规则2）**；单元公式是 OCR 原样**严禁照抄**（步骤 5 重写校正）；单元不经 verify（初版全量保真） |
...
所有 flow（prep → extract → write_source；翻译是 write_source 的内置步骤）的步骤**严格有序、机械不可跳步**。
...
| 内容化 + 拆分（write-source 步骤 3–4） | `flows/write-source/structure/script/build_structure.py`（第 1 步一步产出含内容完整契约，描述信息 `description` 节点 + 条目文字/公式/图片内容块 + 证明 `proof` 子节点按 `sub_sec` 文档顺序挂入分章契约 `_extract/book_structure/ch{N}.json`（附录 `appendix{X}.json`），幂等可重跑；噪声过滤 / 行内公式拼接 / 标题剥离 / 行间公式 tag 键）+ `flows/write-source/script/split_draft_units.py`（步骤 4：由内容化契约拆出 `units/ch{N}/`（附录 `units/appendix{X}/`）每 item 一单元目录 + `manifest.json`，首行 DRAFT 标记；渲染复用 `render_draft.py` 纯函数） |
| MM Repair | `flows/extract/mm_repair/script/mm_repair_audit.py` · `…/mm_repair_text_compare.py` · `…/mm_repair_apply.py` |
| 写作（步骤 5 改单元 + 步骤 7 拼接） | `flows/write-source/script/gate_units.py`（🔴 步骤 5 强制门控：每单元 DONE + **单元级质量校验通过**（check_unit_quality.py：公式闭合 / 围栏形态（单行 `$$` 块、`$$` 附着内容、缺空行·空 `>` 行、`\tag` 出块外）/ 无裸命令·裸 Unicode 字符·裸箭头（数学必须 KaTeX）/ 结构标签 / 块引用结构（嵌套 `> >`、例/证明断裂、blockquote内 bare blank line 打断连续性、`>` 块无标签或标签未包 `>`、列表后无空行、连续空 `>` 行，复用 verify F/H 层九项）/ 无 OCR 残留与内容审阅残留（QED 框「口」/ 乱码重复片段 / 编码损坏 / 单元内私造标题）/ **契约 tag 对账**（`chapter_tag_map` 真值，缺失=漏写、多出=编造）/ **契约 ↔ manifest 双向对账**（正向 = tag/图片对账；反向章级闸 ⑩ = `unit_node_entries` 真值，契约里每个应成单元的节点都须有单元记录，按「习题 / 结果项」分桶 + `norm_secnum`/`sec_ordinals` 序标归一比较，漏记录 = 该条内容 merge 后整条消失）+ **单元标签编号须在契约登记**（质量校验第 23 项，`chapter_ordinals` 真值：行首粗体头 `**2.4.3.**` / `**Exercise 13.3.3\***` / `**习题 13.3.3**` 里的三级序标必须是契约键——OCR 把跨节习题整段挂成 description 时契约无该条目而单元有题面，此前只在步骤 8 verify P 层暴露且标签式形态漏检）/ **契约图片对账 + 空正文闸 + 章级图片覆盖**（`node_images`/`chapter_images` 真值经 manifest `images`/`content` 透传：缺图/多图/正文被整体清空 = FAIL；`unnamed` 占位图与同章标号图别名配对；老 manifest 按 key 唯一回退 + 章级闸兜底）/ **单元结构阅读顺序（页码单调，⑪）**：`lib/unit_order.check_unit_order`（纯函数，与合并后 U 层 `verify/unit_order/` 共用）以契约各节点 `page_start`（原书物理页 = 结构阅读顺序真值）为锚，断言 manifest/合并单元序**页码单调不减**——专补 B 层「同节前缀内单调」与覆盖闸⑩「集合比较任意排列放行」的跨节/跨页盲区；判据：同键取首见页、同页允许、`type=chapter` 跳过、章末 dash 习题（原始键 `N-M`，须在 `norm_secnum` 归一前判，否则 `-`→`.` 与真点分序标 `5.10` 撞车）豁免且不推进游标、锚点缺失跳过不重置游标（判据测试 `lib/tests/test_unit_order.py`）/ 按章批量**真实 KaTeX 渲染**（katex_validate.js，错误映射回单元），「写对」非「重写」，校验执行失败 = 不通过（fail-closed））才 exit 0；🔴 翻译前置硬闸（`--units-dir units-translate`）：先重跑源章门控，源单元未全部修正完成即拒绝——源先于译）+ `flows/write-source/script/merge_units.py`（🔴 步骤 7 纯脚本拼接：**自带强制门控**——拼接前先跑 gate_units，任一单元未改对即直接报错拒绝拼接；按 manifest 合并全部单元成最终 md，按 V-F 重建 `---` 分隔线，无 agent 参与）；格式化 CLI 工具 `tools/wrap_examples_bq` · `tools/fmt_proofs.py` · `verify/format_verify/script/check_katex.py`（KaTeX 检测，由 `verify/format_verify/` 提供）· `verify/format_verify/script/fix_katex.py`（KaTeX 修复，由 `verify/format_verify/` 提供；注：全层 `--fix` 默认禁用，须 `--fix --fix-force` 并通过 PREFLIGHT 围栏门才执行；独立 CLI 内置同款 PREFLIGHT 写回守卫）· `verify/script/audit_counts.py` · `tools/split_chapters.py`（🔴 规则3 超大章按节拆分：合并形态章 md >60000 字符未拆 = `merge_all` 证据复核硬拒 mark，机械闸在 `flows/_flow_contract.py` 的 `merge_all_ok`，判据测试 `flows/write-source/script/tests/test_rule3_split_gate.py`；注：源版「顶层例/证包裹进 `>` + 连续性」由 verify 的 format_verify（F 层）的 `h_mbq` 检测规则及 `{h,h_stmt,h_ul,h_mbq,c,g,…}` 系列 fixer 在 `--fix`（🔴 默认禁用，须 `--fix-force` + PREFLIGHT）自动兜底） |
| 嵌图 | 无独立流程脚本——图片经内容化分章契约 image 块随草稿继承（内容完整性闸门保证齐备）；`flows/script/embed_figures.py` 仅作格式逻辑参考 |
| 校验 | `verify/script/verify_chapter.py` · `config/ignore_chN/manage_ignore.py` |
| 公式对账 | Q 层（`verify/formula_tag/`，opt-in `formula` 配置）覆盖序标集合成员 + 序列顺序(ORDER_MISMATCH) + 小节定位(MISPLACED)；附录字母章位编号 `(A.3)` 须 `formula.letter_ch: true`（`make_config` 对附录页区间自动探测写入，`appendix` 段落 `formula` 键）；公式内容保真人工核对 |
| 结构修复（写源后 remediation） | `tools/restructure_by_ocr.py`：B 层报 ORDERING BLOCKING（条目被放错 §）时，以 OCR `page_*.json` 派生 item→section 真值图 + 贪心单调门把条目归位到源书真实 §（逐字保真、不编造）；先 report 模式核对移动项，确认后 `--apply` 写回（自动备份）。与 write-source「不得重排」规则互补：仅纠正确定性错节，不动正确顺序。**小节锚点扫歪（§N.M.1 被锚到别的小节页）**：根因与预防在 `build_structure`（`_has_foreign_sibling` 异号兄弟否决 + `_toc_band_bottom`/`_hit_below_toc_band` 带下命中原位采用），检测在 `check_structure_completeness.subsection_order_problems`（同父小节键逆序 = 步骤3 BLOCKING，D 层只比 §N.M 对 level 3 失明）。**单元已写毕的旧契约不要重建**（会作废 DONE 单元）：定点修补 = 契约里把节点插回数字键序 + 页码改回现算真锚点，并把该节点的单元记录在 `manifest.json` 里移到同一位置（`merge_units` 按 manifest 顺序拼接，单元文件名与 id 不必随位置排序，正文零损失）；纯标题壳节点（manifest `content=0`）可安全搬动，带内容子块的节点须人工判定归属。修补脚本属特定书，放该书 `_extract/`。 |

各脚本的详细用途与各层 `--fix`（🔴 全层默认禁用，须 `--fix --fix-force` + PREFLIGHT）范围见 [`flows`](flows) 对应流程文档；通用校验文档（层级语义 / 顺序 / 字节契约）见 [`verify`](verify)。
...
- `flows/write-source/script`（`split_draft_units.py`：步骤 4 由内容化分章契约拆出每 item 一单元目录 `units/ch{N}/`（附录 `units/appendix{X}/`）；`gate_units.py`：步骤 5 agent 逐个改好单元后跑强制门控；`merge_units.py`：步骤 7 纯脚本按 manifest 顺序拼接单元成最终 md；`render_draft.py`：单元渲染库——纯渲染函数被 split/merge 复用）。
