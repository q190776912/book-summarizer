...
> - 🔴 **写总结前必须先跑 MM Repair（Step 2.5）**：提取出的 `page_*.json` 是**未修复原料**，必须先经多模态/文本层修复写回，才能据以写总结；哪怕提取已 100% 完结，跳过此步，后续总结无论怎么修都无效。
> - 🔴 **防停滞（规则 B）**：启动任何后台任务（提取 / MM 修复 / 校验）后都**不得停手等用户**——继续做可并行的工作，或原地 `block` 等结果继续推进，绝不对用户说「等通知我继续」把人晾着。
...
- [Workflow（Step 0–4）](#workflow)
  - [🔴 Step 2.5 MM Repair Gate（写总结前强制）](#step25)
...
**规则 B — 防停滞：任何后台任务都不得「停手等用户」（不只提取，最高优先级）**：本 skill 多处用 `run_in_background` 启动长任务（后台提取、MM 修复 audit/apply、批量校验等）。无论启动哪类后台任务，主 Agent **都不得因此停下、把控制权交还用户**：
- ❌ 不得 `TaskOutput(block=True)` 干等后台任务结束（后台任务是另一条线程，主 Agent 不该等它）；
- ❌ 不得「干等完成通知才推进」——通知只是可选提示，不是推进开关；
- ❌ **更不得结束回合、对用户说「等你看完再聊 / 等通知我继续」之类的话**，把用户晾在一边等他来催。
- ✅ 正确做法：启动后台任务后**立即继续做当前能做的并行工作**（提取未完→轮询已落盘页码写初稿；MM 修复未完→先备章节契约/格式规范）；若**当前确实无任何可并行工作**（如整书提取已完、MM 修复是写初稿唯一前置），则**直接 `TaskOutput(block=True)` 原地等结果、在同一回合内继续推进**，不要让用户来推动。
- 本规则是 L267「🚨 主 Agent 行为铁律（防停滞）」的**通用化**：那条原只写在 Step 2 提取轮询语境，但「不得因后台任务停手」对 MM 修复、批量校验等同样适用——上一轮 agent 启动 MM 修复后台任务后直接停手、让用户「等通知」，正是漏看了这条通用化。
...
🔴 **MM Repair 是阶段 1 的硬前置（原料合法性）**：阶段 1 写初稿所依据的 `page_*.json` **必须是已跑完 MM Repair（Step 2.5）并写回修复的版本**。`_extract/_mm_repair/` 不存在，或 manifest 仍存在**未 resolved 的条目**（公式 / deferred 文字仅被模式 B 信任 OCR 或 `text_mode_skipped`、未经模式 A 视觉审读）→ 视为原料不合法，**禁止开始写任何总结**（哪怕提取已 100% 完结）。MM Repair 虽不单列于「四阶段」，但它是阶段 1 的前提，绝非可选项。
...
🔴 **禁止逐章校验（含「用第 1 章做 pilot 提前 verify」的变体）**：阶段 1 写初稿时**不得穿插任何 `verify`**，也**不允许「先写第 1 章、单独 verify 第 1 章来校准格式/配置、再扇出其余章」**——这同样是逐章校验，且更隐蔽，禁止。原因：① 书级配置 `verify_config.json`（规则 H）在**阶段 2、即全部源语言初稿写完后**才生成；D 层 `section_depths`、Q 层 `formula` 序标映射、ordinal 分组都依赖**全书**编号形态，单章 verify 时配置尚未定型 → 结果失真、属**无效功**；② A–Q 若干判定需整章/整书上下文，单章跑既慢又误导。唯一正确顺序：**先写完全书源语言初稿 → 阶段 2 生成完整配置 → 阶段 3 用 `verify_chapter.py --all` 一次性批量校验**。全书完成后的最终汇报一次性给出。
...
🚫 **明确禁止「写完一章就校验一章」**：阶段 1 写源语言初稿期间**不做任何 per-chapter verify**；配置生成（阶段 2）与批量校验（阶段 3）统一在所有源语言初稿完成后进行。判定不清时，先抽一页原文 / 看 TOC 确定编号与小节层级，再生成文件，**不得依赖静默默认值**。⚠️ `verify_chapter.py` 的**单章模式（`<ch> <start> <end> <md> <extract>`）仅供阶段 3 批量失败后「定位单章问题」用，绝不可在阶段 1 当「边写边查」工具调用**；阶段 1 唯一允许的产出是 `.md` 初稿 + 嵌图。
...
> **🔴 总结前必须先执行的步骤**。分三种模式（A / B / C）；**WorkBuddy 是多模态模型，文本类书固定走「混合策略 B→A」，模式 A（视觉审读）必走**——纯模式 B 不在正常路径内，仅当部署确实无视觉能力才作极端兜底。
> - **模式 A（多模态 / agent 视觉审读，🔴 对 WorkBuddy 强制）**：WorkBuddy 本身支持看图（多模态），**模式 A 必走，不存在「模型不支持看图」的退路**。读 `page_NNN_sheet.png` + manifest 逐条修正。
...
>
> **🔶 混合策略（文本类书的标准做法，WorkBuddy 固定走此路径）**：文本类书**先跑模式 B**把「文本层干净的文字」修准/确认（标 `mm_repaired`/`mm_reviewed`），把**公式 + 文本层不可信的文字**保持未解决；随后 **WorkBuddy 支持看图，模式 A 强制**：**重跑 audit**（已修文字因 page JSON 标记被跳过，公式/不可信文字会重新标出）→ 走**模式 A** 由 agent 视觉读取真实印刷页处理剩余项（公式 + deferred 文字必须逐条过视觉，**不得直接交规则 G 跳过**）。这样把便宜准确的文本层补偿吃干抹净，只把贵且慢的视觉识别用在它不可替代的公式与坏文本层区域。
> ⚠️ **「纯模式 B」仅是极端兜底**（仅当该部署确实无视觉能力时才可用）。正常 WorkBuddy 部署**必须走到模式 A**；纯模式 B 把不可信文字按「信任 OCR」记 `ok` 收尾、公式交规则 G 的做法，对多模态 agent 属于「跳过视觉识别」的偷懒，**禁止采用**。若因环境所限不得不采用纯模式 B，必须在 `_extract/_mm_repair/` 下显式写 `MM_NO_VISION` 标记且用户知情。
>
...
> 🔴 **提取已 100% 完结（无后台提取可轮询）时也必须跑**：本书若在上个会话已提取完毕、`_extract/page_*.json` 全部落盘，Step 2 轮询循环会因「无新页可轮询」被自然跳过——**但 MM Repair 不能随之跳过**。此时直接对**全书所有页码**跑一次完整链路（audit → 模式 B → 重跑 audit → 模式 A → apply），再进入 规则 E 阶段 1 写初稿。触发条件（页码增长）只是「增量并行」的优化路径，**不是「是否要做」的开关**。
...
2.5 **模式判定（选择 A / B / C）**：审计已标出条目后，按顶层「三种模式」说明分流：
   ...
   - 书籍为文本类（PDF 有可选中数字文本层，`mm_repair_text_compare.py` 自动探测通过或用 `--force-text`）：
     - **混合策略（推荐，WorkBuddy 固定走此路径）**：先走 **模式 B（步骤 3b，加 `--hybrid`）** 修准/确认文字；随后 **WorkBuddy 支持看图，模式 A 强制** → **重跑 audit** → 走 **模式 A（步骤 3a）** 处理公式与不可信文字残留（必走，不可省略）。
     - （极端兜底，仅部署确实无视觉能力时）走 **模式 B（步骤 3b，不加 `--hybrid`）**，不可信文字按信任 OCR 记 `ok` 收尾，公式交 summary 规则 G。WorkBuddy 正常部署不在此列。
...
3b. **【模式 B】文本层补偿**（文本类书，无需看图）：用 `mm_repair_text_compare.py` 从 PDF 抽取低置信 `text` 条目所在区域的干净数字文本，与 OCR 文本比对并自动产出 `repairs.json`：
   ...
   - 公式条目无文本层可取 → **保持未解决**，标记 `text_mode_skipped`（仅作信息），**不在此模式补偿**；WorkBuddy 支持看图，**必须交模式 A 视觉审读**，不可直接以「否则规则 G」为由跳过（规则 G 只兜底真正的公式残留，不替你免掉文字层的视觉修正）。
...
> ```
> （极端兜底，仅部署确实无视觉能力）省略 `--hybrid` 与步骤 4–6，模式 B 把不可信文字记 `ok`、公式交规则 G。WorkBuddy 正常部署**必须保留步骤 4–6（模式 A）**。
...
> 本 skill 对英文原著书默认产出「英文版 + 中文版」两文件。两文件**不是平行独立写作**，而是**英文版为源、中文版为派生**。下面的 1–5 是**全书两趟批量**的执行顺序（严格遵循 规则 E，**不是**「写完一章就跑完 1–5 再写下一章」的逐章循环）：**第一趟**先对**全部章节**执行 1–2（写英文源版 + 批量校验英文，写初稿期间禁止任何 per-chapter verify）；**第二趟**再对**全部章节**执行 3–4（据已校验英文派生中文 + 批量校验中文）。目录级脚本（`wrap_examples_bq` / `fmt_proofs` / `embed_figures`）只在两趟收尾时由主控制器统一跑，禁止各 agent 并发期运行（见 规则 E 多 agent 注记）。
...
- **公式**：用 `$$`（前空行），编号 `	ag{N.M}`，禁止代码围栏包裹；KaTeX 渲染须成功；**公式序标（编号 1:1 真实性 / 编造·错位·跨章·遗漏）规则见 [`references/formatting.md#公式序标`](references/formatting.md#公式序标)**
...
🔴 **本步只在 规则 E 阶段 3/4 触发，禁止前移**：`verify_chapter.py --all` 是**全部源语言初稿写完后**的批量关卡。阶段 1 写初稿期间**绝不**调用本步（含其单章模式）——见 规则 E「禁止逐章校验」。提前单章 verify 时书级配置尚未定型（D 层 `section_depths` / Q 层 `formula` 序标映射 / ordinal 分组都依赖全书编号形态），结果必然失真、纯属无效功。
