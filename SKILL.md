...
> 每个阶段是一个独立 `flows/<name>/<name>.md`（统一模板：目的 / 前置 / 步骤 / 本阶段规则 / 出口 / 相关代码 / 子流程）。点开链接看该阶段的完整规则与命令。通用校验 `verify` 是**顶层公用文档**（与 `flows` 并行，路径 `verify`），被 `verify-source` 与 `derive-translate` 复用，不属于某个流程阶段。
