...
- `flows/write-source/script`（`split_draft_units.py`：步骤 4 由内容化分章契约拆出每 item 一单元目录 `units/ch{N}/`（附录 `units/appendix{X}/`）；`gate_units.py`：步骤 5 agent 逐个改好单元后跑强制门控；`merge_units.py`：步骤 7 纯脚本按 manifest 顺序拼接单元成最终 md；`render_draft.py`：单元渲染库——纯渲染函数被 split/merge 复用，不再单独产出整章草稿）。
...
约定：
- `flows/<stage>/ref/` 下除 SSOT 文档外，各流程的脚本索引写在流程文档「相关代码」节（不再维护单独的 script/README.md）。
