...
| 配置文件 | 作用 | 生成 / 维护 | 说明文档 |
|---------|------|------------|------|
| `verify_config.json` | 书级配置**外层 map**：`{"ch": {...}, "appendix": {...}, "supplement": {...}}`，每个子配置含编号形态 `ordinal`（含图序标 Figure 组）、语言 `language`、公式序标 `formula`、忽略章节 `ignore` 等。`ch`=正文章（kind=1），`appendix`=附录章（kind=2）、`supplement`=补篇章（kind=3）可选；`verify_chapter.py` / `scan_skeleton` 的**唯一配置源**，`ConfigLoader` 按 kind 路由 | `verify_config/make_config.py` 扫对应页区间半自动生成（文件已存在且非 `--force` 时跳过，不会覆盖手动修改），或手填 | [`verify_config/verify_config.md`](./verify_config/verify_config.md) |
...
| `ignore_ch{N}.json` / `ignore_fig_ch{N}.json`（附录章文件名段为 `appendix{X}`、补篇章为 `supplement{S}`，如 `ignore_appendix{A}.json` / `ignore_supplement{S}.json`，判据 `chapter_label`） | 每章 verify 忽略键（条目级）/ 图片噪声豁免键（图片级） | 手填（`--ignore` / `--ignore-figure`）或 `ignore_chN/manage_ignore.py` | [`ignore_chN/ignore_chN.md`](./ignore_chN/ignore_chN.md) |
