...
1. **分组语义从「模式开关」变成「数据声明」**：旧 `separate_types=0/1` 控制「全共享计数器 vs 每类独立计数器」；新模型把分组信息编码进 `ordinal` 数组的每个元素（via `name`）。两侧（提取侧 `../flows/extract/script/b_layer.py` 与 MD 侧 `verify/b_layer.py`）的 `_group_key` 逻辑都要替换为 `name → group` 匹配。
...
- 复用既有：`lib.regexlib`、`config` 常量（`ORDINAL_*`、`ORDINAL_DEPTH`、`ORDINAL_STRUCTURE`、`ORDINAL_LANGUAGE_DEFAULT`、`ORDINAL_SECTION_TYPES`）、`verify/key_parse.py` 的 `_canon_label` / 各 `*_RE`。
...
| `../flows/extract/script/b_layer.py` | **分组逻辑** | `_group_key(it, separate_types)` → `_group_key(it, groups)`（name→group 匹配）；删除 `from verify.script.b_layer import SEP_*`；`recover_missing_items(groups=...)` |
| `verify/b_layer.py` | **MD 侧分组** | `_md_gap_blocking` 逐组解析（label→group→该组 depth）+ 分组键 `(gi\|prefix)`；移除 `SEP_*` import/使用；`cfg.depth` → `group.depth`；`group_prefix_len()` → `group.prefix_len()` |
...
| `verify/extract_layer.py` | **派发** | `cfg.extractor_family()` 选提取器；`extract_items(groups=cfg.ordinal)`；`keys_in_md(md, cfg.ordinal, ...)`；`_merged_category_first_missing` 网关改 `cfg.has_cn_three_level()` |
| `verify/base.py` | **属性** | `ordinal` property 改为返回 `self.config.ordinal`（list），并更新 `extract_layer`/`p_layer`/`d_layer` 的 int 比较调用点改用 `cfg.extractor_family()` / `cfg.primary_type()` |
| `verify/p_layer.py` | **适配** | `check_bare_items(lines, ordinal)` → `check_bare_items(lines, family)`（传 `cfg.extractor_family()`） |
| `verify/d_layer.py` | **适配** | `cfg.ordinal in (GM,ROMAN)` → `cfg.primary_type() in (GM,ROMAN)`；`ORDINAL_SECTION_TYPES.get(cfg.ordinal)` → `ORDINAL_SECTION_TYPES.get(cfg.primary_type())` |
...
| `../verify/item_numbering_integrity/item_numbering_integrity.md`、`../verify/verify.md`、`references/layers/extract.md` | **文档同步（建议，P2）** | 同步分组模型说明（非阻塞，可后置） |
...
见 `ordinal_groups_class.mermaid`。
...
### 5.1 `BookConfig.from_dict`（config/verify_config/verify_config.py）
...
def from_dict(cls, data):
    ...
    if isinstance(raw, int):                      # 旧 {"ordinal": int} 废弃
        raise ConfigError(
            ...
            "请重跑 config/verify_config/make_config.py 重新生成配置。")
...
- 删除 `from verify.script.b_layer import SEP_COMBINED, SEP_PER_TYPE`。
...
**MD 侧 `verify/b_layer.py`：**
- 删除 `from config import SEP_COMBINED, SEP_PER_TYPE` 及模块内 SEP 注释块。
...
4. **`SEP_*` 删除的波及**：`SEP_COMBINED/SEP_PER_TYPE` 在 `config.py`、`verify/b_layer.py`、`../flows/extract/script/b_layer.py` 三处定义/引用。删除时三处须同步，且 `flows/write-source/format/ref/separator_regexlib.md:124` 曾标注「严禁改动 SEP 常量」。
...
### T01 — 核心数据模型（`../config/verify_config/verify_config.py`）【P0，无前置依赖】
**文件**：`../config/verify_config/verify_config.py`
...
**文件**：`../flows/extract/structure/script/extract_items.py`、`../flows/extract/script/b_layer.py`、`verify/b_layer.py`、`verify/key_parse.py`
...
- `verify/b_layer.py`：删除 `SEP_*` import/使用；`_md_gap_blocking` 逐组解析（`match_group`→`group.depth`，gm/roman 用 roman/gm 解析器）；`cfg.depth`/`group_prefix_len()`/`cfg.separate_types` 全部替换为 `group.prefix_len()`；分组键 `f"{gi}|{prefix}"`。
...
**文件**：`verify/extract_layer.py`、`verify/base.py`、`verify/p_layer.py`、`verify/d_layer.py`、`../flows/extract/structure/script/scan_skeleton.py`、`verify/verify_chapter.py`、`../config/verify_config/make_config.py`
...
**文件**：`../config/verify_config/tests/test_config_complete.py`
