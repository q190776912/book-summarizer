...
- **允许适度精简**：精简的是表述不是内容——压缩冗余铺陈，但不得省略任何编号项，也**不得省略描述性内容（动机/直观/导语等）中的公式与概念**。书源所有带编号公式（含描述性散文中的）的保留要求见下方**硬性要求 7**
...
- 🔴 **小节序标必须尊重原书（SSOT）**：小节标题的编号形态**以原书为准**，本技能不强制任何固定小节编号格式——原书怎么写就怎么写。
  ...
  - 配套配置：此类书须在 `_extract/verify_config.json` 的 `section_types` 数组里把**章层级和小节层级都写为 `0`**（无序号标角色，对应 `type 0` / `depth 0`）。Silverman 章是文件（`# 第N章`，无 `## §` 编号）、文件内 `## § <标题>` 小节也无编号，故写 `"section_types": [0, 0]`（第一个 `0` = 章层级、第二个 `0` = 小节层级，两个层级都无序号标；默认带序标书是 `[1, 2]` 之类）。`section_types` 含 `0` 时，verify 的缺节闸门按「位置/数量」比对结构契约（只查契约要求的节是否都在 md 里、不计 md 多出的小节），**不强求加回原书没有的序标**。这与其他带序标书（默认按数字逐一对齐）零回归。
...
### 7. All Book-Source Numbered Formulas Must Be Retained (Including Descriptive Prose)

教材中**所有带编号的公式**——无论出现在定理/定义/例题的**陈述**里，还是出现在章节**描述性散文 / 推导过程**中——都必须 1:1 出现在总结中，并挂对应的 `\tag{<原书编号>}`。

- **编号公式不分"陈述级"还是"描述级"**：凡是书源以 `(N.M)`、`(N)`、`(N.M.K)`、`(N.M-N)` 等形式带编号呈现的公式，都是必须保留的对象。描述性推导式（如投影/矩阵运算、Krylov 递归、DMD/EDMD/GLA 公式）同样计入，**不得以"精简"为由跳过**。
- **禁止以"精简"为由省略带编号公式**：这与「核心原则·允许适度精简」不冲突——精简只压缩冗余铺陈，**绝不**省略编号公式（含描述性推导式）。省略编号公式 = 违反本规则 + rule 10（公式编号 1:1 跟书）。
- **唯一合法的"省略"路径**：若某编号公式经判断确实可省略（如纯排版重复、跨章复述），必须在 `verify_config.json` 的 `formula.ignore` 中**显式登记**该编号。未登记的遗漏会被 Q 层（公式序标校验）判为 MISSING 并以 **FAIL（阻断）** 报告——不再以 WARN 放行。
- **检查**：Q 层 `formula_tag` 对书源编号集合 S 与总结中 `\tag` 集合对账；任何属于本章、未在 `ignore` 登记、且总结中无对应 `\tag` 的编号 → MISSING → **FAIL（阻断）**。
...
> ⚠️ **校验（verifier）口径——重要修正**：上面的"省/留"是**内容收录**规则，校验口径要分两类，不能一刀切：
> - **带有专门习题小标题的集中习题块**（如 `Exercises` / `## §1.11 习题` / `**练习**` 下罗列的编号题）→ 既不写入总结，也**不计入 verifier 必备条目**（其 `type:"exercise"` 节点在 verify 真相集中被排除，以 `consolidated:true` 标记）。
> - **无标题穿插、被保留下来的练习** → 既写入总结（原位写出题目），**也应纳入 verifier 编号项校验**——它们是真实存在的编号条目，漏写 / 跳号应当被发现。
>
> 因此 `book_structure.json` 中虽含全量 `type:"exercise"` 节点，verify 只应核对**被保留的那部分**练习节点；集中块练习节点须标 `consolidated:true` 并在 verify 真相集中排除。早期"练习一律不校验"的表述是错的——错在把"集中块"和"被保留的练习"混为一谈。
...
10. **公式编号必须用 `\tag{<原书编号>}` 写在 `$$` 块内部**：当原书公式带有编号，总结中必须将编号以 `\tag{...}` 的形式附加在公式行末尾（闭合 `$$` 之前），使渲染后编号显示在公式右侧同一行。这里的 `\tag{}` 只是 KaTeX 渲染公式序号的**编码机制**——它只强制"编号须进 `$$` 块、随公式同行显示"这一**呈现方式**，并不规定编号本身的形态；`<原书编号>` 的内容（层级、分隔符、是否含字母）**必须逐字照搬原书**：原书单段 `(7)` → `\tag{7}`，两段点分 `(1.17)` → `\tag{1.17}`，三段点分 `(11.1.1)` → `\tag{11.1.1}`，短横 `(11.1-1)` → `\tag{11.1-1}`。**字母 / 罗马开头的编号（如 `(A.3)` / `(I.2)`）作者仍须按书原样写 `\tag{A.3}`——但注意：Q 层公式序标校验（opt-in）目前只校验「数字开头」编号，`(A.3)` 这类校验逻辑尚未实现（见 SSOT 第 4 条）；含此类编号的书跑 verify 时 Q 层会显式 **FAIL（blocking）** 而非判通过——因为逻辑没写、无法校验即不可通过，须先补完「字母/罗马前缀」支持才能放行，不得用 WARN 轻轻带过假绿。**本技能**不规定任何固定编号格式**，书上怎么写就怎么写（编号 1:1 真实性见下文 SSOT 第 2–3 条）。**禁止**将编号写成公式块外部的独立文字行（如 `（式 (1.17)）`）。
    - ✅ 正确：
      ```
      $$
...
> 🔴 **总原则（高于一切具体条目）**：公式编号的**形态以原书为准，本技能不规定任何固定编号格式**——书上怎么写就怎么写。`\tag{}` 仅是 KaTeX 编码机制，括号内的 `<原书编号>`（层级、分隔符、是否含字母）须**逐字照搬原书**；本规则禁止为"统一风格"而把原书编号改形（如把 `(11.1-1)` 改成 `(11.1.1)`、把单段 `(7)` 硬凑成两段）。

总结中带编号的公式（`$$ ... \tag{<原书编号>} $$`）其序标必须与**原书公式编号 1:1 对应**（`<原书编号>` 的形态逐字照搬原书，不从固定模板）：
...
4. **字母 / 罗马开头的编号（`(A.3)` / `(I.2)`）目前会使 Q 层 FAIL（blocking，须先补逻辑）**：`SourceFormulaIndex.norm()` 与 `build_formula_patterns()` 当前只识别「数字开头」编号（N / C.N / C.S.N / C.S-N / 尾字母 `C.Na`），`(A.3)` 这类首段为字母 / 罗马数字的正则匹配不到、`norm()` 返回 `None`，会被抽空 → 集合 S 空 → 而散文里的 `3.1 Theorem` 又会让 S 非空、字母编号被静默忽略。为避免这种**假绿**，只要书源抽到任意字母 / 罗马开头的编号，`_detect_letter_led_formulas` 就会触发、**Q 层对该章判 FAIL（blocking）**——因为校验逻辑没写、无法验证 1:1 真实性即不可通过，**不得报"公式校验通过"**，须先为 `norm()` 与 `build_formula_patterns()` 增加「可选首段 `[A-Za-z]{1,4}[.\-·,]` 前缀 + 罗马数字处理」支持（代码已留 TODO(letter-led) 锚点），逻辑落地后探测自动退场转为真校验。作者在此之前仍须按书原样写 `\tag{A.3}`（呈现方式不受限）。
