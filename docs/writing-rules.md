...
- **习题（练习）收录规则**（判定标准 = 有无专门习题小标题归拢）：
  ...
  - **问题（Problem）与练习同通道**：`problem` 类型节点（如 Lee 2e 章末 Problems，独立编号计数器、独立 `Problem` 组）与穿插练习同一收录机制与写作规则；是否省略**只由契约节点的 `consolidated` 标记承载**（未标 `consolidated` 的问题节点逐条原位写出），写手不自行按书中小标题判断省略。
  - ⚠️ 校验口径：集中习题块节点标 `consolidated:true` 并在 verify 真相集排除（既不写也不校验）；被保留的穿插练习与问题节点**既写也纳入编号项校验**（真实条目，漏写/跳号应被发现）。
...
- **🔴 粗体行内标签，禁 `###` 条目标题**：定义/定理/命题/推论/引理/公理/注记写成 `**标签**：`；例用块引用 `> **例…**：`。`###` 只用于 `##` 下的真子节分隔，且必须带 `§` 前缀。
  - 原书小节带序标 → `## §3.2 节名`；**三级子节**（序标 3 段，如 `3.2.1`）→ `### §3.2.1 节名`，**四级** → `#### §3.2.1.1 …`（头部 `#` 数 = 序标段数，且 `###`/`####` 必须带 `§` 前缀）；无序标（如 Silverman）→ `## § <描述性标题>`（数字留空，不编造）。
...
> 🔴 **反向规则（真实代码用 ``` 围栏）**：本条只禁"用代码围栏包公式 / 交换图表"。书本里**真实的计算机代码**（计算机代数系统命令如 Mathematica `In[1]:= ...`、算法伪代码、REPL 输入输出等）**必须用 ```` ```lang ```` 代码围栏包裹**，不要用行内反引号 `` ` ``；在块引用(`>`)内写作 `> ```lang` … `> ``` `。公式与交换图表仍走 KaTeX（本条），绝不用代码围栏包公式。
