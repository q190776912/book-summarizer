...
This is the **detection phase** of the skill's two-phase figure pipeline.
  ...
  Phase 1 (this module) — DETECTION (runs inside the recognition flow):
...
Two entry points:
  * run_full_book(pdf, out_dir, ...) -> detect on ALL pages (1..N), assign each
        ...
        write a single global ``figure_detect.json``. Called by the pipeline.
...
def parse_fig_label(text):
    """Extract '图 X.X.X' style label from caption text.

    Also accepts English 'Figure X.X' so figures with English captions still
    get a friendly name (used by the assignment phase, not detection).
    """
    if not text:
        return None
    m = re.search(r"图\s*([0-9]+(?:(?:\.|-)[0-9]+){1,2})", text, re.IGNORECASE)
    if m:
        return m.group(1)
    m = re.search(r"(?:figure|fig\.)\s*([0-9]+(?:(?:\.|-)[0-9]+){1,2})", text, re.IGNORECASE)
    if m:
        return m.group(1)
    return None
...
def _write_detect(out_dir, entries, md_lines):
    with open(os.path.join(out_dir, "figure_detect.json"), "w", encoding="utf-8") as f:
...
def detected_pages_set(out_dir):
    ...
    if not os.path.exists(dp):
        return set()
    try:
        entries = json.load(open(dp, encoding="utf-8"))
...
def detect_pages_range(pdf_path, out_dir, start, end, model=None,
    ...
    for pno in range(start, end + 1):
        ...
        e, _ = process_page(model, arr, W, H, ocr_items, fig_id, cap_id,
                            ch, pno, fig_dir, conf, imgsz, device)
        new_entries += e
    doc.close()
    ...
    if os.path.exists(dp):
        try:
            existing = json.load(open(dp, encoding="utf-8"))
        except Exception:
            existing = []
