...
## 触发
- 父流程 `extract` 轮询循环中，`current_max_page` 自上次 MM audit 以来增长时（增量并行）；
- **或**提取已 100% 完结、无新页可轮询时，也必须对**全书所有页码**跑一次完整链路（触发条件只是"增量优化路径"，不是"是否要做"的开关）。

## 前置
- PDF 路径与 `_extract` 目录已知。
- 父流程 `extract` 轮询循环运行中（或已完结进入"全书补跑"）。
