...
核心思路：
  ...
  - 公式条目无文本层可取 → **保持未解决**并标记 text_mode_skipped，不在本模式
    补偿；后续若有视觉能力则交模式 A，否则 summary 阶段按 write-source 流程「理解后重写」原则处理。
...
from repairs import Repairs  # repairs.json instantiation lives with its model
...
def run(pdf_path, extract_dir, src_dpi, expand_pt, force_text=False, force_scan=False,
    ...
    repairs = {"corrections": {}, "ok": {}, "to_structured": {}, "deferred": {}}
    ...
    for pstr, page_info in manifest.get("pages", {}).items():
        ...
        page_data = PageJson.load(pf).data if os.path.isfile(pf) else {}
        ...
        for e in page_info.get("entries", []):
            ...
            if kind == "formula":
                ...
                # 仅作信息），以便后续重跑 audit 重新标出交给模式 A / write-source 流程「理解后重写」原则。
    ...
    Repairs(**repairs).dump(repairs_path)
