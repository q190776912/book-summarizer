...
# SEP_COMBINED/SEP_PER_TYPE constants are gone — see config/verify_config/verify_config.py GroupConfig.)
...
def recover_missing_items(extract_dir, chapter, start_page, end_page, items, label_re, TAIL_GAP_THRESHOLD, cfg=None):
    ...
    for idx, sec_key in enumerate(sec_order):
        ...
        # detection runs on the written .md in verify/item_numbering_integrity/script/item_numbering_integrity.py.
        if first_num > 1:
