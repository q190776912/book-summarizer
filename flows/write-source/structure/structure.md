...
- 内容块自动处理：页眉 / 页脚 / 版权行 / 页码噪声过滤；行内公式（MFD `cls=0`）按 x 位置拼回宿主文本行恢复阅读序；行间公式同行右缘的编号 tag（`(2.17)` 独立文本块）挂到公式块 `tag` 键并从散文流剔除——**跨章守卫**：`formula.scope==2`（章级编号）且章键为数字/单字母时，候选 tag 首段（按 `.`/`-`/`–` 分割）若 ≠ 当前章号则**拒绝挂载**（如第 3 章页面上 OCR 碎片 `2-1`、跨章引用 `定理2.1` 不会被误挂成本章编号公式；`scope==3` 节级编号、`appendix` 等非数字章键不触发守卫）；条目 / 小节正文开头按序匹配契约 `name` 的归一化前缀时剥离印刷标题（宁重复不误删）；证明标记 / QED 识别失败不拆（宁整不碎）。
...
- **🔴 断块交叉引用幻影守卫（`extract_items_en3.py`，2026-09-22 Katok 实测）**：三级英文书 `Label C.S.N` 条头判据「块首锚定 + 号后句点/大写」在 OCR 把一句拆成两块时会误升**句中交叉引用**为**同号重复条目**，产幻影节点（真条头在别处已抽到，`dedup_items` 因两现文本不同而全保留）。两种形态与对策：(1) **跨块连字拼接的无连字符分支**——上块尾恰为完整标签词、下块以裸号起（`…as in Lemma` + `2.4.7. If f is close…` 拼成幻影 `Lemma 2.4.7`，与 p94 真条头同号二现）；仅当上块该词之前的文本**为空或以句读终止**（`. : ; 。 ： ! ?`）才允许无连字符拼接，句中尾（`…as in`）拒绝（连字符断词 `Proposi-`+`tion` 分支不受限）。(2) **块首回指**——交叉引用标签词正落在新块块首（`…the assertion of` | `Proposition 2.1.3. Then…`）；`_emit` 内若匹配在真块首、上一块**明显停在句中**（末字符为小写字母/逗号）且该 key **本章已首次抽到**，判为回指丢弃。判据保守：真·首现条头 key 未见过、合法同号重印通常起于句读边界，均不受影响。全书 22 章回归：0 全量丢失、0 新增、14 处 `2→1` 去幻影且保留的皆为真陈述页。
