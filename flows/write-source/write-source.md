...
## 目的
依据已修复的 `page_*.json`，按原书结构逐条写出**源语言**章节总结（英文书 = 英文 `ChapterN_*.md`；中文书 = 中文 `第N章_*.md`）。本阶段只产出源语言初稿 + 嵌图，**不做任何校验**。
## 前置
...
2. 按 `docs/writing-rules.md` 全部格式规则写**源语言**文件：标题体系（`# 第N章` / `## §N.M` / `### §N.M.K`）、粗体条目标签（`**Definition 1.1**` 禁 `###`）、块引用、分隔线、`$$` 公式、图片嵌入（见子流程 figures）、练习收录。
3. 写完后跑**格式后处理**（顺序固定）：
   ```powershell
   python flows/write-source/script/wrap_examples_bq <book_dir>   # 顶层例包进 > 块引用（须先跑）
   python flows/write-source/script/fmt_proofs <book_dir> --number # 块引用/分隔线/证明格式修复
...
- **规则3 — 超大章按"节"拆分**（字符 > 60000 触发，中英文配对拆分）：`flows/write-source/script/split_chapters` 按节标题格式（`§N` 节标题式 / `N.M` 编号式）拆成每节一个文件；只拆到"节"一级，子节留父节内。幂等，拆分后默认删源合并文件（`--keep` 保留）。
...
- `flows/write-source/script/wrap_examples_bq` / `flows/write-source/script/fmt_proofs`：格式后处理（KaTeX 检测/修复已并入 verify，见 `verify/format_verify/script/check_katex.py` 与 `fix_katex.py`，由 `verify --fix` 自动修复）。
- `../../verify/script/audit_counts.py`：逐节条数核对（强制收尾）。
- `flows/write-source/script/split_chapters`：规则3 拆章。
