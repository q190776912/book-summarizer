...
## 目的
依据已修复的 `page_*.json`，按原书结构逐条写出**源语言**章节总结（英文书 = `ChapterN_*.md`；中文书 = `第N章_*.md`）。本阶段只产出初稿 + 嵌图，**不做任何校验**（校验在 Stage 4 批量进行）。

## 触发
- 父流程 `extract` 轮询循环中，某章 `info.end <= current_max_page` 且已过 MM Repair 门（extract/mm_repair 流程 规则1）时。

## 前置
- 该章 `page_*.json` 已落盘且过 MM Repair（extract/mm_repair 流程 规则1 满足）。
- 已知该书的源语言（write-source 规则1：英文书源=英文版，中文书源=中文版）。

## 步骤（有序）
1. **消费 extract 末尾已生成的契约**：`ch<N>_skeleton.txt`（结构骨架契约）与 `ch<N>_items.txt` / 扫描报告（编号项清单）已在 extract 阶段 Step 5 / Step 6 批量生成，直接读取——骨架有几个 `SEC` 就写几个 `## §N.M`，顺序照抄；每个 `ITEM` 必须落地；编号项清单与骨架交叉核对，确保无遗漏。
...
- **规则2 — 写初稿期间禁止任何 per-chapter verify**：本阶段唯一产出是 `.md` 初稿 + 嵌图；`verify` 统一在源语言全部初稿完成后的 Stage 4 批量进行。🚫 禁止"写完一章 verify 一章"，也禁止"第 1 章 pilot verify 后扇出"。
...
## 相关代码（路径相对 skill 根目录）
- `flows/extract/script/extract/scan_skeleton`：结构骨架契约（extract Step 5 已批量生成 `ch<N>_skeleton.txt`，此处直接消费）。
- `flows/extract/script/extract/extract_items` + `scan_items.py`：编号项清单（extract Step 6 已批量生成；英文书 `--lang en`）。
