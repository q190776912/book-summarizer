...
| DocLayout-YOLO weight (`doclayout_yolo_ft.pt`) | `D:\study\model\PDF-Extract-Kit\models\Layout\YOLO\doclayout_yolo_ft.pt`（图片提取 `figure/extract_figures.py` 用；从 ModelScope `opendatalab/pdf-extract-kit-1.0` 拉，HF 此环境不可达） |
