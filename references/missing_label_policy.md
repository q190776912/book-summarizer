...
> 由用户 2026-08-05 制定，配套代码实现见 [`layers/extract.md`](../verify/references/layers/extract.md) 与 `verify/layers/extract_layer.py` 的"merged Q + over-mark 守卫"段；本文件只描述**策略与判定**，不重复代码。
...
1. **调 OCR 参数**（`extract/extract_items.py` 或抽取脚本）：
   - `--dpi`：`200` / `300` / `400`（清晰度）。
...
3. **登记抽取覆盖**：在本章 `_extract/manual_overrides_ch{N}.json` 中追加该键（`key` / `page` / `label` / `text`），使抽取器承认其存在、解除 B 层序列缺口 BLOCKING。这是 skill 既定的"agent 恢复的 OCR 条目"机制（`extract_items.py` 第 350 行注释；覆盖项会被打 `agent_recovered` 标记）。
...
B 层本就是"查漏"层。下列两项原拟作独立 Q / R 层，经用户 2026-08-05 指示**并入 B 层**（消除独立层与其 `q_cat_missing` 契约键）。
...
- `extract/b_layer.py` 的序列连续性检查：某节抽取到 `…-1,2,3,5,6,7` 而缺 `-4` 时，先重扫页面；重扫无果 → 仍判 `blocking`（"still missing after auto-recovery"）。
- 这正是 4.9-4 被吞时的实际触发点；用 §2 Step 2 的 `manual_overrides` 登记后即解除。
- **（与 B 层 MD 侧检测的关系）** 提取侧的此 `blocking` 属辅助检测，最终受 B 层 `b_layer.py` 的「MD 存在性过滤」约束：若被报缺的编号实际已正确写在 `.md` 中（OCR 漏检、agent 已写出），该 `blocking` 会被抑制、不阻断——故「OCR 漏检但 .md 已写对」不会误报。权威的缺号判定见 [`layers/b.md`](layers/b.md)（MD 侧首项检验 + 连续性）。
...
- 代码：`verify/layers/extract_layer.py`（`_scan_book_category_items` / `_merged_category_first_missing` / `_merged_ocr_overmark_guard`）。
- 抽取覆盖：`extract/extract_items.py`（`manual_overrides` 合并 + `agent_recovered` 标记）、各书 `_extract/manual_overrides_ch{N}.json`。
- 注册表：[`verification.md`](../verification.md) 第 1 节（B 层）、第 3 节同步清单（本策略无新契约键，仅复用 `blocking`/`warnings`）。
