...
🔴 度量口径（按渲染行测量，勿按逐源码行测量）
------------------------------
「逐源码行取 max(vis_len)」的口径对「一个渲染行被折成多行书写」的公式系统性漏报：
同一公式在源码里占多行、每行都很短（如阈值 100 下最长源行仅 39），整行渲染宽度
却可能远超阈值（如 71）——肉眼明显过宽却判为合格。只有恰好写成单行的公式才会被抓到。

统一复用 `tools/scan_long_formulas.py` 的度量（单一事实来源，避免多处实现分叉）：
