...
from formula_manifest import parse as parse_summary, normalize as normalize_summary
...
def reconcile_chapter(chapter, extract_dir, cmap, md_root, out_dir=None,
    ...
    if out_dir:
        ...
        kreyszig_map.KreyszigMap(data=result).dump(os.path.join(out_dir, f'recon_ch{chapter}.json'))
    return result
...
def main():
    ...
    if args.out and results:
        ...
        kreyszig_map.KreyszigMap(data=agg).dump(os.path.join(args.out, 'recon_all.json'))
