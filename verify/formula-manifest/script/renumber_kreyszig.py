...
from reconcile_kreyszig import build_book_manifest, build_summary_manifest, align_section
...
def main():
    ...
    cmap = chapter_map.load_chapter_map_raw(args.chapter_map)
    bk_root = args.md_root
    out_dir = os.path.join(_ROOT, 'verify/formula-manifest/script', '_recon', 'md_bak')
    ...
    map_file = os.path.join(_ROOT, 'verify/formula-manifest/script', '_recon', 'renumber_map.json')
    kreyszig_map.KreyszigMap(data=all_maps).dump(map_file)
