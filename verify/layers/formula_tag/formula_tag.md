...
> **注册机制**：本层脚本位于 `verify/layers/formula_tag/script/formula_tag.py`，由 `verify/script/register_all.py` 用 `importlib` 按裸名扫描 `verify/layers/*/script/` 自动发现并注册。`code = 'Q'` 是稳定字母代号（被 SKILL.md 与 per-book 记忆广泛引用，**不可更改**）；新增层无需改 `register_all.py` / `VerifyManager` / CLI。
...
## 出口条件
FABRICATED / INCONSISTENT 非空 → 整章 FAIL；MISSING 仅 WARN。

## 相关代码（`verify/layers/formula_tag/script/formula_tag.py`）
- `code = 'Q'`，`order = 17`（当前最大层 P=16 之后），`auto_fixable = False`。
- 经 `verify/script/register_all.py` 用 `importlib` 按裸名扫描 `verify/layers/*/script/` 自动发现注册，**无需改 register_all.py / VerifyManager / CLI**。
...
`Q-LAYER FORMULA MISSING` → 仅 WARN（非阻断）。
- **修复步骤**：
  ...
  4. S 为空降级（配置错）→ 回头修正 `formula.depth/scope` 让书源抽到编号，不要当成“通过”。
  5. 重跑 verify，确认 FABRICATED / INCONSISTENT 为空。
