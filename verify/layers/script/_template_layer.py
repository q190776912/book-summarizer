...
使用方法：
  1. 复制本文件为 `X_layer.py`（`X` 为大写字母，且不能与**现有层的 code** 重复；
     完整 code 列表与运行顺序见 [`verify/verify.md` 的「校验层注册表（SSOT）」](verify/verify.md)）。
  ...
  3. 实现 `run(self, ctx) -> LayerResult`（返回本层贡献的结果字典键）。
     ...
     `fix_dict`（键名即 fix 契约键，须与 verify/layers/script/base.py 的合并逻辑、report.py 的
     展示顺序约定保持一致）。
  4. 本文件以 '_' 开头，register_all.py 的自动发现会跳过它，因此它不会被当作一层。

⚠️ 若你的层要贡献**新的**结果字典键（即 DEFAULT_RESULT 里还没有的键），
   必须同步更新（完整步骤见 [`verify/verify.md` 的「新增一个校验层（子包约定）」](verify/verify.md)）：
     - `verify/layers/script/base.py` 的 `DEFAULT_RESULT`（增加该键及正确类型的占位值），
     - `verify/script/report.py` 的 `print_result`（增加该键的展示逻辑），
     - `verify/verify.md` 的「校验层注册表（SSOT）」索引表与「字节契约键集合」清单，
...
manual_path, ignore_keys, ignore_fig，以及配置 ordinal（由 config.BookConfig
...
class TemplateLayer(VerifyLayer):
    ...
    code = 'X'            # 稳定标识符，单大写字母，禁止与现有层的 code 重复（见 verify/verify.md 校验层注册表）
    order = 99            # TODO: 运行顺序，设为当前最大 order +1（完整列表见 verify/verify.md 校验层注册表）；不要写死成固定总数
