...
> **注册机制**：本层脚本位于 `verify/layers/section_continuity/script/section_continuity.py`，由 `verify/script/register_all.py` 用 `importlib` 按裸名扫描 `verify/layers/*/script/` 自动发现并注册。`code = 'D'` 是稳定字母代号（被 SKILL.md 与 per-book 记忆广泛引用，**不可更改**）；新增层无需改 `register_all.py` / `VerifyManager` / CLI。
...
## 相关代码（`verify/layers/section_continuity/script/section_continuity.py`）
