...
- **七道闸门**：
  ...
  4. `p_missing_sec`：缺节（md `## §` 数 < 结构契约 section 数；契约见 `flows/extract/structure/structure.md`，由 `build_structure.py` 产出的 `ch<N>_structure.json`）。
  5. `p_extra_item`：编造条目（md 出现结构契约编号项清单没有的编号条目）。
