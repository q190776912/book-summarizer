...
# Standalone single-component formula: bare (N) / （N） NOT preceded by a word
# or CJK char (so x(0)/f(0) function-call parens are rejected).
_F_SINGLE_RE = re.compile(r'(?<![\w\u4e00-\u9fff])[（(]\s*(\d+)\s*[）)]')

# Dotted two-component in parentheses: (C.N) / （C.N）.
_F_DOT_RE = re.compile(r'(?<![\w\u4e00-\u9fff])[（(]\s*(\d+\.\d+)\s*[）)]')

# EN dotted references: Eq. C.N / Equation C.N.
_F_EQ_RE = re.compile(r'\b(?:Eq\.?|Equation)\s+(\d+\.\d+)')

# CN dotted reference: 式（C.N）/ 式 C.N.
