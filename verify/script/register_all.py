...
机制：扫描 verify/layers/ 下每个 <snake>/script/<snake>.py（每个校验层目录的 script/
子目录），按裸名 import（boot.setup() 已将 **/script 注入 sys.path），收集其中定义的
VerifyLayer 子类并注册。层文档 <snake>.md 位于 verify/layers/<snake>/ 目录内（与 script/ 并列）。
共享 helper（base.py、_fig_common.py、_struct_labels.py、_template_layer.py、
_reconcile_book_formulas.py）留在 verify/layers/script/，不会被当作层。运行顺序完全由各层
...
from verify.layers.script.base import LayerRegistry, VerifyLayer

LAYER_REGISTRY = LayerRegistry()

# 发现机制：每个校验层是一个目录 verify/layers/<snake>/，脚本在 verify/layers/<snake>/script/<snake>.py。
# 层文档 <snake>.md 位于 verify/layers/ 级。共享 helper 留在 verify/layers/script/。
# boot.setup() 已将所有 **/script 注入 sys.path，故层脚本可按裸名 import。
# 注意：verify/layers/ 无 __init__.py（命名空间包，__file__ 为 None），故层根目录必须基于本文件的
# 绝对位置推导（register_all.py 位于 verify/script/，故 parents[1] 即 verify/，再拼 'layers'），
# 不可依赖 verify.layers.__file__。
_LAYERS_ROOT = Path(__file__).resolve().parents[1] / 'layers'  # .../verify/layers
