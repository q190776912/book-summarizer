...
支持**任意嵌套深度（1–4 级序标：`## §N` / `## §N.M` / `## §N.M.K` / `## §N.M.K.L`）**，由 `<book>/_extract/verify_config.json` 的 `section_types` 驱动——`section_types` 是**逐层级列表（从章层级排到最深 `## §` 层级）**，每个元素是**该层级的序标段数**（1/2/3/4 段），`0` 表示该层级无序号标；深度经 `SECTION_TYPE_DEPTH` 派生（详见 `../verify.md` 的 section_types 字段说明）。**列表长度 = 章节层级总数（章计入）**，故单层级书为 `[0]`、章+无序号标小节书为 `[0, 0]`（两个层级，Silverman 即此）。「章/节/小节/子小节」只是标准书里这些段数的*典型称呼*，并非硬语义；一本书可声明 `section_types: [1,1,1]`（所有层级都是一级序标）或 `section_types: [0, 0]`（章与章下小节都无序号标，如 Silverman）。未显式声明时按 `ordinal` 反推（`ORDINAL_SECTION_TYPES`）：ordinal 2/4/6 校验一级+二级序标两级（节基 EN 两级 = type 4 + `chapter_first:false`，同样走两级）；ordinal 3/5 额外校验三级序标（1.1.1）层级；ordinal 1 仅一级序标。

> 节的**条目级**尾部缺口（节在、末号丢了）仍由 **B 层** `_md_tail_warnings` 负责（md 末号 vs 抽取契约）。D 不重复做条目级尾部，只做“整节”层面的两件事。

### 全局单序标路径（`sections_global: true`，Arnold《数学方法》型）
`§1..§52` 跨章连续、首分量非章号的书中，通用投影的「token 首分量==章号」守卫毫无意义（md 每个标题都被丢弃、只剩 OCR 噪声造幻影尾节）。该标志开启专用路径 `_check_d_layer_global`：
- 数字节验收 = **契约节号 ∪ md 已写** 交集内的单数字头（OCR 碎片/跨章引用不可能伪造节）；
- `section_types` 含 role `5` 时第三级校验裸字母子块：md 侧 `### §A` / `### §N.A`（历史投影形态兼容），源侧复用 scan_skeleton 的 `SUB_GLOBAL` 守卫（汉字起始 + 禁句读 + 块宽 ≤720px）并与契约 `letter_subs` 取交集；内部断号落 continuity、越尾落 missing，渲染为 `N.A`；
- 附录章（契约节键全字母）：`## §A` 字母节直接按 id 对齐。
缺契约时退化为空转（不报不拦），缺节检测由 P 层数量闸兜底。
