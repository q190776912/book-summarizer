...
Fixtures live in ``verify/tests/fixtures/q_formula/`` (kreyszig_ch3.md +
kreyszig_ch3_ext/page_*.json).  The real corpus is read-only, never modified.

Run:
    python verify/tests/test_q_layer_formula_regression.py
    python -m pytest verify/tests/test_q_layer_formula_regression.py -q
"""
import os
import re
import io
...
MAKE_CONFIG_PY = os.path.join(SKILL_ROOT, 'verify', 'make_config.py')
...
# unsafe under pytest's output capture).  These mirror verify/make_config.py.
_F_SINGLE_RE = re.compile(r'(?<![\w\u4e00-\u9fff])[（(]\s*(\d+)\s*[）)]')
_F_DOT_RE = re.compile(r'(?<![\w\u4e00-\u9fff])[（(]\s*(\d+\.\d+)\s*[）)]')
_F_EQ_RE = re.compile(r'\b(?:Eq\.?|Equation)\s+(\d+\.\d+)')
_F_CN_EQ_RE = re.compile(r'式\s*[（(]?\s*(\d+\.\d+)')
    ...
    def test_global_detection_scans_all_pages(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            code = (
                "import sys, json;"
                "sys.path.insert(0, %r);"
                "from verify.make_config import detect_formula;"
                "print(json.dumps(detect_formula(sys.argv[1])))" % SKILL_ROOT
            )
