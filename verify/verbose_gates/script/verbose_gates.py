...
# 2026-09-26（Kreyszig ch1）扩展：容忍标号前的列表符号与粗体/斜体标记——
# 原书按 (a)(b)(c)(d) 分步的证明（Kreyszig 1.6-2 完备化定理）写成
# `> **(a) Construction of ...**` 块引用步是合规排版，旧判据只认裸 `(a) `，
# 把已分条的长证明误判成「逐段翻译墙」，写手被迫放弃印刷步标号。
STEP_RE = re.compile(
    r'^\s*(?:[-*>]\s+)?[*_]*\s*(?:\d+\.|[（(]\d+[)）]|[（(][a-zA-Z][)）])'
    r'[\s:*_\u4e00-\u9fff]')
...
def _source_ngrams(ext_dir, ch, n=8):
    ...
    try:
        ...
        chunks = []
        _pdir = resolve_page_dir(str(ext_dir), ch)
        for pg in range(start, end + 1):
            fp = os.path.join(_pdir, "page_%03d.json" % pg)
